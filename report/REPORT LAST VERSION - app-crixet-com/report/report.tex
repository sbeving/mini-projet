...
Application logs constitute the primary source of truth for system health, security posture, and operational intelligence. The average enterprise generates terabytes of log data daily across its infrastructure --- from web servers and databases to authentication systems and network devices. \placeholderfig{Enterprise Log Volume Growth (2015-2025)}{log_volume_growth.png}{Bar chart showing exponential increase from 1TB/day to 50TB/day}

As illustrated in Figure \ref{fig:log_volume_growth.png}, the data volume expands at a CAGR of approximately 25\%, creating a significant "data noise" challenge for traditional auditing methods.
...
In the early days of computing, log management was a manual process. System administrators would periodically review text files using command-line tools such as \texttt{grep}, \texttt{awk}, and \texttt{tail}. Figure \ref{fig:log_evolution_timeline.png} presents the evolution of log management technologies, highlighting the transition from manual file inspection to automated, AI-driven analysis.
...
The Elastic Stack --- comprising Elasticsearch, Logstash, and Kibana --- represents the industry standard for open-source log management. Figure \ref{fig:elk_architecture.png} illustrates its architecture.
...
Figure \ref{fig:siem_comparison_radar.png} presents a radar chart visualization of the comparison.
...
Figure \ref{fig:nodejs_event_loop.png} illustrates the Node.js event loop architecture.
...
Figure \ref{fig:postgres_indexing.png} illustrates the indexing strategy employed for log queries.
...
Figure \ref{fig:go_binary_size.png} compares binary sizes across languages for equivalent functionality.
...
Figure \ref{fig:ollama_architecture.png} shows the Ollama integration architecture.
...
Figure \ref{fig:scrum_timeline.png} presents the sprint timeline and deliverables.
...
Figure \ref{fig:actors_context.png} presents the context diagram showing actor interactions.
...
Figure \ref{fig:usecase_global.png} presents the comprehensive use case diagram depicting all actors and their interactions with the LogChat platform. This visualization emphasizes the central role of the AI Chat in daily analyst operations.
...
Figure \ref{fig:sequence_login.png} details the complete authentication sequence from user input to dashboard redirect.
...
Figure \ref{fig:sequence_rag_chat.png} illustrates the RAG (Retrieval Augmented Generation) process, which is central to LogChat's AI capabilities.
...
Figure \ref{fig:sequence_log_ingestion.png} presents the complete flow from agent transmission to threat detection and dashboard notification.
...
Figure \ref{fig:activity_threat_detection.png} models the decision flow within the threat detection engine.
...
Figure \ref{fig:class_diagram_erd.png} presents the domain model with all entities, attributes, and relationships.
...
Figure \ref{fig:architecture_global.png} presents the global system architecture showing all components and their interactions. The separation of the Agent from the core stack ensures minimal performance impact on monitored hosts.
...
Figure \ref{fig:component_diagram.png} details the internal structure of each containerized service.
...
Figure \ref{fig:deployment_diagram.png} illustrates the production deployment topology.
...
Figure \ref{fig:data_flow_diagram.png} presents the end-to-end data flow from log sources to end users.
...
Figure \ref{fig:database_schema.png} presents the physical database schema with all tables and relationships.
...
Figure \ref{fig:auth_architecture.png} illustrates the JWT-based authentication architecture.
...
Figure \ref{fig:agent_architecture.png} presents the internal architecture of the Golang agent.
...
The system employs the Strategy pattern for collectors, enabling polymorphic handling of different log sources. The base interface and common functionality are detailed in Appendix \ref{app:code_collector}.

Figure \ref{fig:collector_class_diagram.png} shows the collector class hierarchy.
...
The Windows collector utilizes direct Win32 API calls to interface with the Event Log system. The implementation uses the \texttt{advapi32.dll} library for low-level access (see Appendix \ref{app:code_windows}).

Figure \ref{fig:windows_eventlog_flow.png} illustrates the Windows Event Log collection flow.
...
Figure \ref{fig:sender_state_machine.png} shows the Sender state machine.
...
Figure \ref{fig:backend_architecture.png} presents the backend component architecture.

\placeholderfig{Backend Component Architecture}{backend_architecture.png}{Express → Routes → Services → Prisma → PostgreSQL}

The authentication service manages user login and JWT generation. It utilizes \texttt{bcrypt} for password verification and creates audit sessions for security tracking (see Appendix \ref{app:code_auth}).

\subsection{Threat Detection Engine}

\begin{lstlisting}[language=Javascript, caption=Pattern-Based Threat Detection]
...
Figure \ref{fig:threat_detection_patterns.png} presents the threat pattern categories.
...
The RAG engine retrieves relevant logs based on user queries, constructs a prompt with this context, and interfaces with the LLM provider. The detailed implementation is provided in Appendix \ref{app:code_rag}.

Figure \ref{fig:rag_workflow_detail.png} illustrates the RAG workflow in detail.
...
Figure \ref{fig:screenshot_dashboard.png} shows an actual screenshot of the dashboard.
...
Figure \ref{fig:screenshot_chat.png} presents the AI chat interface screenshot.
...
Figure \ref{fig:testing_pyramid.png} illustrates the testing pyramid employed for LogChat quality assurance.
...
Figure \ref{fig:test_coverage_report.png} shows the test coverage report.
...
Figure \ref{fig:performance_charts.png} presents the performance test visualizations.
...
Figure \ref{fig:docker_network.png} presents the Docker network topology.
...
Set strong JWT\_SECRET & Min. 256-bit random \\
Set strong DB\_PASSWORD & Min. 16 characters \\
Configure backup for postgres\_data & Daily automated \\
Set up log rotation & logrotate config \\
Configure firewall rules & Allow 3000, 3001, 11434 \\
Pull LLM model & \texttt{ollama pull qwen2.5:0.5b} \\
Run database migrations & \texttt{npx prisma migrate deploy} \\
...
\placeholderfig[0.95]{Alert Management Screenshot}{screenshot_notifications.png}{Alert dashboard showing threat alerts with severity and status indicators}
...
\chapter{Appendix E: Source Code Excerpts}

This appendix contains the full source code listings referenced throughout the implementation chapters.

\section{Golang Agent: Collector Interface}
\label{app:code_collector}
...
\section{Golang Agent: Windows Event Log}
\label{app:code_windows}
...
\section{Backend: JWT Authentication}
\label{app:code_auth}
...
\section{Backend: RAG Chat Workflow}
\label{app:code_rag}
...
router.post('/', authMiddleware, async (req, res) => {
    ...
    const systemPrompt = `You are LogChat, an AI analyst.
Analyze the following log data and answer the user's question.
LOG CONTEXT: ${buildLogContext(logs, stats)}
`;
